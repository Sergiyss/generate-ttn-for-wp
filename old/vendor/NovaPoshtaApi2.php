...
class NovaPoshtaApi2 {
    ...
    function getWarehouses($cityRef, $page = null, $limit = null, $filter = null) {
    ...
    protected function checkInternetDocumentRecipient(array & $counterparty) {
        ...
        if ( ! $counterparty['RecipientsPhone'])
            ...
            throw new \Exception('RecipientsPhone is required filed for recipient');
    ...
    function newInternetDocument($sender, $recipient, $params) {
        ...
        if ( ! $sender['Sender']) {
            ...
            $sender['RecipientWarehouseIndex'] = setRecipientWarehouseIndex($recipient['CityRecipient'], $recipient['AddressNP']);
            ...
            if ($senderCounterpartyExisting['data'][0]['Ref']) {
                ...
                $sender['SendersPhone'] = $sender['SendersPhone'] ? $sender['SendersPhone'] : $contactSender['data'][0]['Phones'];
        ...
        $counterpartyData = $this->model('Counterparty')->save(
            array(
                'FirstName' => $recipient['FirstName'],
                'MiddleName' => $recipient['MiddleName'],
                'LastName' => $recipient['LastName'],
                'Phone' =>  $recipient['RecipientsPhone'],
                'Email' => '',
        ...
        $CityRef = $this->request('Address', 'getCities', 
            array(
                'FindByString' => $recipient['CityRecipient'],
                'Limit' => '20'
            ),
        );
        // Проверка на успешный запрос и наличие данных
        if ($CityRef['success'] && isset($CityRef['data'])) {
            // Цикл по данным
            foreach ($CityRef['data'] as $city) {
                // Сравнение Description и DescriptionRu
                if ($city['Description'] == $recipient['CityRecipient'] || $city['DescriptionRu'] == $recipient['CityRecipient']) {
                    // Если они полностью совпадают, вернуть Ref
                    echo "Список городов: ref ". $city['Ref'];
                    $recipient['CityRecipient'] = $city['Ref'];
                    break;
                }
            }
        }
    ...
    function setRecipientWarehouseIndex($wcus_city_ref, $addressName){
        return $this->request(
                    'Address', 'getWarehouses', array('CityRef' => $wcus_city_ref, 'FindByString' => $addressName));
